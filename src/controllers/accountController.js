...
const login = async (req, res) => {
   ...
   try {
      const [rows] = await connection.query("SELECT * FROM users WHERE phone = ? AND dial_code = ?", [phoneNumber, dialCode]);
      if (_.isEmpty(rows)) {
      ...
      await connection.execute("UPDATE users SET token = ?,login_at= ? WHERE phone = ? ", [md5(accessToken), checkTime, phoneNumber]);
      return res.status(200).json({
...
const register = async (req, res) => {
   try {
      ...
      while (true) {
         const [rows] = await connection.query("SELECT `id_user` FROM users WHERE `id_user` = ?", [id_user]);
      ...
      const [userRegOtp] = await connection.query("SELECT * FROM users_regi WHERE phone = ? ORDER BY id DESC LIMIT 1", [phoneNumber]);
      ...
      const sql = "INSERT INTO users SET id_user = ?,phone = ?,name_user = ?,password = ?,plain_password = ?, money = ?,bonus_money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?,dial_code = ?,login_at=0";
      await connection.execute(sql, [id_user, phoneNumber, name_user, hashedPassword, pwd, 0, bonus_money, code, invitecode, ctv, 1, otp, ip, 1, time, dialCode]);
      ...
      await connection.execute("UPDATE `users` SET `token` = ? WHERE `phone` = ? ", [md5(accessToken), phoneNumber]);
       await connection.execute("DELETE FROM users_regi WHERE phone = ?", [phoneNumber]);
...
const sendUserRegOtpCode = async (req, res) => {
   try {
      const schema = Joi.object({
      ...
      let beijingTimestamp = Math.floor(utcTime.valueOf() / 1000);
      const generateSignature = (apikey = "yFflv72N", secret = "HAfMWKuM") => {
         const key = apikey + secret + beijingTimestamp;
      ...
      if (response?.data?.status === "0") {
         ...
         await connection.execute(
            "INSERT INTO users_regi (otp, time_otp, phone) VALUES (?, ?, ?)",
            [otp, timeEnd, phone]
         );
...
const resetPasswordByOtpAndPhone = async (req, res) => {
   try {
      ...
      const [rows] = await connection.query("SELECT `otp`, `time_otp` FROM users WHERE `phone` = ? AND veri = 1", [phone]);
      ...
      if (Number(user.otp) === Number(otp)) {
         ...
         await connection.execute("UPDATE users SET password = ?, plain_password = ? WHERE phone = ? ", [hashedPassword, newPassword, phone]);
         return res.status(200).json({
...
const resetPasswordByPassword = async (req, res) => {
   try {
      ...
      const hashedPassword = await bcrypt.hash(newPassWord, saltRounds);
      await connection.execute("UPDATE users SET password = ?, plain_password = ? WHERE phone = ? ", [hashedPassword, newPassWord, user.phone]);
...
const updateUsernameAPI = async (req, res) => {
   try {
      ...
      await connection.execute("UPDATE users SET name_user = ? WHERE token = ?", [nickname, auth]);
...
const updateAvatarAPI = async (req, res) => {
   try {
      ...
      await connection.execute("UPDATE users SET avatar = ? WHERE token = ?", [avatar, auth]);
      return res.status(200).json({
