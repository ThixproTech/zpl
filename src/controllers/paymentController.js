...
import { sendAdminNotification } from "../server.js"; // ✅ Add .js extension
...
const initiateManualUPIPayment = async (req, res) => {
  ...
  return res.render("wallet/manual_payment.ejs", {
    Amount: query?.am,
    UpiId: momo.upi_id,
    qr_code: momo.usdt_wallet_address,
  });
};
...
const addManualUPIPaymentRequest = async (req, res) => {
  ...
  try {
    ...
    const utr = data.utr?.trim(); // keep string for length check
    const minimumMoneyAllowed = parseInt(process.env.MINIMUM_MONEY_INR);
    ...
    if (!utr || utr.length < 11 || utr.length > 12) {
      return res.status(400).json({
        message: `UPI Ref No. or UTR is Required and it should be 11 or 12 digits long`,
        status: false,
        timeStamp: timeNow,
      });
    }
    ...
    console.log("UPI Ref No. or UTR is already used");
    ...
    if (pendingRechargeList.length !== 0) {
      await Promise.all(
        pendingRechargeList.map((recharge) =>
          rechargeTable.cancelById(recharge.id)
        )
      );
    }
    ...
    console.error("❌ UPI manual payment error:", error.stack || error);
  ...
  // catch (error) {
  //   console.error("UPI manual payment error:", error);
  //   return res.status(500).json({
  //     status: false,
  //     message: "Something went wrong!",
  //     timestamp: new Date().toISOString(),
...
const verifyOnlinePayment = async (req, res) => {
  try {
    ...
    const [info] = await connection.query(
      `SELECT * FROM recharge WHERE id_order = ? ORDER BY id DESC LIMIT 1`,
      [orderId]
    );
    if (statusp === "SUCCESS") {
...
// --------------------------------------------
// Update the amount and utr
const updateRechargeAmount = async (req, res) => {
  try {
    const id = parseInt(req.body.id, 10);
    const amount = parseFloat(req.body.amount);
    const utr = req.body.utr;

    if (isNaN(id) || isNaN(amount) || !utr) {
    ...
    const result = await connection.query(query, [amount, utr, id]);
    ...
    return res.json({ message: "Update Amount Successfully." });
  } catch (error) {
...
const setRechargeStatus = async (req, res) => {
  try {
    ...
    if (data.status === PaymentStatusMap.SUCCESS) {
      ...
      const [get2BonusLevel] = await connection.query(
        "SELECT first_deposite_bonus_level FROM recharge WHERE phone = ? ORDER BY id DESC LIMIT 2",
        [recharge.phone]
      );

      let first_deposite_bonus_level = 0;

      // Ensure that at least two rows exist before accessing index 1
      if (get2BonusLevel.length >= 2) {
        first_deposite_bonus_level +=
          get2BonusLevel[1].first_deposite_bonus_level;
      }

      if (first_deposite_bonus_level === 0) {
        first_deposite_bonus_level = 1;
      }

      await connection.query(
        "UPDATE recharge SET status = 1, first_deposite_bonus_level = ? WHERE id = ?",
        [first_deposite_bonus_level, data.id]
      );
...
const walletTransfer = async (req, res) => {
  try {
    ...
    // sender start
    // games win and loss 
    const [betWinResult] = await connection.query(
      "SELECT SUM(winAmount - betAmount) AS total_win_amount FROM igtechbets WHERE phone = ? AND winAmount > betAmount",
      [user.phone]
    );

    const [betLossResult] = await connection.query(
      "SELECT SUM(betAmount) AS total_loss_amount FROM igtechbets WHERE phone = ? AND winAmount = ?",
      [user.phone, 0]
    );

    const [result_5dWinResult] = await connection.query(
      "SELECT SUM(get - money) AS total_win_amount FROM result_5d WHERE phone = ? AND status = ?",
      [user.phone, 1]
    );

    const [result_5dLossResult] = await connection.query(
      "SELECT SUM(money) AS total_loss_amount FROM result_5d WHERE phone = ? AND status = ?",
      [user.phone, 2]
    );

    const [k3WinResult] = await connection.query(
      "SELECT SUM(get - money) AS total_win_amount FROM result_k3 WHERE phone = ? AND status = ?",
      [user.phone, 1]
    );

    const [k3LossResult] = await connection.query(
      "SELECT SUM(money) AS total_loss_amount FROM result_k3 WHERE phone = ? AND status = ?",
      [user.phone, 2]
    );

    const [wingoWinResult] = await connection.query(
      "SELECT SUM(get - money) AS total_win_amount FROM minutes_1 WHERE phone = ? AND status = ?",
      [user.phone, 1]
    );

    const [wingoLossResult] = await connection.query(
      "SELECT SUM(money) AS total_loss_amount FROM minutes_1 WHERE phone = ? AND status = ?",
      [user.phone, 2]
    );

    const [trxWingoWinResult] = await connection.query(
      "SELECT SUM(get - money) AS total_win_amount FROM trx_wingo_bets WHERE phone = ? AND status = ?",
      [user.phone, 1]
    );

    const [trxWingoLossResult] = await connection.query(
      "SELECT SUM(money) AS total_loss_amount FROM trx_wingo_bets WHERE phone = ? AND status = ?",
      [user.phone, 2]
    );
    // games win and loss

    const [withdrawResult] = await connection.query(
      "SELECT SUM(money) AS total_withdraw_amount FROM withdraw WHERE phone = ?",
      [user.phone]
    );

    const [balanceTransferResult] = await connection.query(
      "SELECT SUM(amount) AS total_transfer_amount FROM balance_transfer WHERE sender_phone = ?",
      [user.phone]
    );

    let totaltrxWingoWinResult = parseFloat(trxWingoWinResult[0].total_win_amount) || 0;
    let totaltrxWingoLossResult = parseFloat(trxWingoLossResult[0].total_loss_amount) || 0;
    let totalwingoWinResult = parseFloat(wingoWinResult[0].total_win_amount) || 0;
    let totalwingoLossResult = parseFloat(wingoLossResult[0].total_loss_amount) || 0;
    let totalk3WinResult = parseFloat(k3WinResult[0].total_win_amount) || 0;
    let totalk3LossResult = parseFloat(k3LossResult[0].total_loss_amount) || 0;
    let totalResult_5dWinResult = parseFloat(result_5dWinResult[0].total_win_amount) || 0;
    let totalResult_5dLossResult = parseFloat(result_5dLossResult[0].total_loss_amount) || 0;
    let totalApiGameWinAmount = parseFloat(betWinResult[0].total_win_amount) || 0;
    let totalLossAmount = parseFloat(betLossResult[0].total_loss_amount) || 0;
    let totalWithdrawAmount = parseFloat(withdrawResult[0].total_withdraw_amount) || 0;
    let totalTransferAmount = parseFloat(balanceTransferResult[0].total_transfer_amount) || 0;
    let totalWithdrawAndTransferAmount = totalTransferAmount + totalWithdrawAmount;
    let totalWinAmount = totaltrxWingoWinResult + totaltrxWingoLossResult + totalwingoWinResult + totalwingoLossResult + totalk3WinResult + totalk3LossResult + totalResult_5dWinResult + totalResult_5dLossResult + totalApiGameWinAmount + totalLossAmount;
    let saveWinAmount = totalWinAmount - totalWithdrawAndTransferAmount;

    // console.log("Total Win Amount", totaltrxWingoWinResult);
    // console.log("Total Loss Amount", totaltrxWingoLossResult);
    // console.log("Total Win Amount", totalWinAmount);
    // console.log("Total Loss Amount", totalLossAmount);
    // console.log("Total Tran With Amount", totalWithdrawAndTransferAmount);
    // console.log("Total Save Amount", saveWinAmount);

    if (saveWinAmount < amount) {
      return res.status(400).json({
        message: `Insufficient Balance. Your Winning Amount is ${saveWinAmount < 0 ? 0 : saveWinAmount.toFixed(2)}`,
        status: false,
        timeStamp: new Date().toISOString(),
      });
    }

    const senderHalfPerBonus = 0;
    const senderAmount = amount - senderHalfPerBonus;

    await connection.query(
      "UPDATE users SET money = money - ? WHERE phone = ?",
      [senderAmount, user.phone],
    );
    // sender end

    // receiver start
    const [recharge] = await connection.query(
      "SELECT COUNT(*) AS count_recharge FROM recharge WHERE phone = ?",
      [phone]
    );

    const [receiverBalanceTransferResult] = await connection.query(
      "SELECT COUNT(*) AS count_transfer FROM balance_transfer WHERE receiver_phone = ?",
      [phone]
    );

    let countRecharge = parseInt(recharge[0]?.count_recharge) || 0;
    let countTransfer = parseInt(receiverBalanceTransferResult[0]?.count_transfer) || 0;
    let totalCount = countRecharge + countTransfer; // Combine both counts
    let receiverBonusAmount = 0;

    // if (totalCount === 0) {
    //   receiverBonusAmount = amount * 0.05; // 5%
    // } else if (totalCount === 1) {
    //   receiverBonusAmount = amount * 0.03; // 3%
    // } else if (totalCount === 2) {
    //   receiverBonusAmount = amount * 0.02; // 2%
    // } else {
    //   receiverBonusAmount = amount * 0.01; // 1%
    // }
    let receiverAmount = amount + receiverBonusAmount;
    await connection.query(
      "UPDATE users SET money = money + ? WHERE phone = ?",
      [receiverAmount, phone],
    );

    if (countRecharge > 0) {
      await connection.query(
        "UPDATE recharge SET money = money + ?, p2p_money = p2p_money + ?, remaining_bet = remaining_bet + ?, first_deposite_bonus_level = first_deposite_bonus_level + ? WHERE phone = ? ORDER BY time_remaining_bet DESC LIMIT 1",
        [receiverAmount, receiverAmount, receiverAmount, 1, phone],
      );
    } else {
      await connection.query(
        "INSERT INTO `recharge` (`phone`, `money`, `p2p_money`,  `first_deposite_bonus_level`, `remaining_bet`, `transaction_id`, `type`, `time`, `time_remaining_bet`, `today`, `status`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          phone,
          receiverAmount,
          receiverAmount,
          receiverAmount,
          1,
          "Send By " + user.phone,  // Ensure proper spacing
          "Recharge By P2P",
          rechargeTable.getCurrentTimeForTimeField(),
          rechargeTable.getCurrentTimeForTimeField(),
    ...
    await connection.query("INSERT INTO `balance_transfer` (`sender_phone`, `receiver_phone`, `amount`, `sender_bonus`, `receiver_bonus`, `datetime`) VALUES (?, ?, ?, ?, ?, ?)",
      [user.phone, phone, amount, senderHalfPerBonus, receiverBonusAmount, moment().format("YYYY-MM-DD HH:mm:ss")],
    );
    ...
    const mailOptions = {
      from: "testing.thixpro@gmail.com",
      to: "testing.thixpro@gmail.com",
      subject: "Money Transfer Notification",
      text: `📢 Transfer Alert!  
    
User 📱 ${senderPhone} has transferred ₹${amount} to 📱 ${receiverPhone}.  
...
✅ Thank you.`,
    ...
    sendAdminNotification({
      ...
      message: `User 📱 ${senderPhone} has transferred ₹${amount} to 📱 ${receiverPhone}`,
    });
...
const getInviterTeamByInviteCode = async (inviteCode) => {
  if (!inviteCode) throw new AppError("invite code not provided", 400);
  const [inviterTeam] = await connection.query(
    `SELECT phone FROM users WHERE invite = ?`,
    [inviteCode]
  );

  return inviterTeam || null;
};

const getInviterTeamRechargeByPhone = async (phone) => {
  let todayDate = moment().format("YYYY-DD-MM");
  if (!phone) throw new AppError("phone not provided", 400);
  const [inviterTeamRecharge] = await connection.query(
    `SELECT SUM(money) AS total_money FROM recharge 
      WHERE phone = ? AND status = ? AND DATE(today) = ?`,
    [phone, 1, todayDate]
  );

  return inviterTeamRecharge?.[0] || null;
};
...
const getUserByInviteCode = async (invite) => {
  ...
  const [inviter] = await connection.query(
    "SELECT phone, invite, code FROM users WHERE `code` = ?",
    [invite]
  );
...
const addUserAccountBalance = async ({ money, phone, invite, rechargeId }) => {
  ...
  const latestRechargeAmount = await rechargeTable.amountRecharge(
    PaymentStatusMap.SUCCESS,
    phone
  );

  console.log(latestRechargeAmount);

  // const firstRechargeBonus = totalRecharge === 1 ? (money / 100) * 10 : 0;
  const firstRechargeBonus =
    totalRecharge === 1 ? (money * 0.3) / 100 :
      totalRecharge === 2 ? (money * 0.2) / 100 :
        totalRecharge === 3 ? (money * 0.1) / 100 :
          0;

  // const franchiseBonus = latestRechargeAmount === 50000 ? (money / 100) * 5 : 0;
  // const user_money = money + firstRechargeBonus + franchiseBonus;
  ...
  // if (franchiseBonus !== 0) {
  //   await addUserRewards(phone, franchiseBonus, REWARD_TYPES_MAP.FRANCHISE_BONUS);
  // }

  const inviter = await getUserByInviteCode(invite);

  if (inviter) {
    const inviterFirstRechargeBonus =
      totalRecharge === 1 ? (money / 100) * 3 : 0;

    if (inviterFirstRechargeBonus !== 0) {
      await addUserRewards(
        inviter.phone,
        inviterFirstRechargeBonus,
        REWARD_TYPES_MAP.FIRST_RECHARGE_AGENT_BONUS
      );

      await addUserMoney(inviter.phone, inviterFirstRechargeBonus);
    }
  }

  const inviterInviteCode = inviter.code;
  const inviterTeam = await getInviterTeamByInviteCode(inviterInviteCode);
  const directTeamLength = inviterTeam.length;

  let teamRecharge = 0;
  const rechargePromises = inviterTeam.map(async (inviterTeam) => {
    const inviterTeamRecharge = await getInviterTeamRechargeByPhone(
      inviterTeam.phone
    );
    return parseFloat(inviterTeamRecharge?.total_money) || 0;
  });

  const rechargeResults = await Promise.all(rechargePromises);
  teamRecharge = rechargeResults.reduce((sum, money) => sum + money, 0);

  const inviterPhone = inviter.phone;
  const salaryBonusThresholds = [
    { directTeamLength: 1, teamRecharge: 600, bonus: 50 },
    { directTeamLength: 2, teamRecharge: 1200, bonus: 100 },
    { directTeamLength: 3, teamRecharge: 1800, bonus: 150 },
    { directTeamLength: 4, teamRecharge: 2400, bonus: 200 },
    { directTeamLength: 5, teamRecharge: 3000, bonus: 250 },
    { directTeamLength: 10, teamRecharge: 6000, bonus: 500 },
    { directTeamLength: 15, teamRecharge: 9000, bonus: 750 },
    { directTeamLength: 20, teamRecharge: 12000, bonus: 1000 },
    { directTeamLength: 25, teamRecharge: 15000, bonus: 1250 },
    { directTeamLength: 30, teamRecharge: 18000, bonus: 1500 },
  ];

  for (const {
    directTeamLength: minTeam,
    teamRecharge: minRecharge,
    bonus,
  } of salaryBonusThresholds) {
    if (directTeamLength > minTeam && teamRecharge > minRecharge) {
      const getUserRewardBonus = await getUserRewards(
        inviterPhone,
        bonus,
        REWARD_TYPES_MAP.DAILY_SALARY_BONUS
      );
      console.log(getUserRewardBonus);

      if (getUserRewardBonus < 1) {
        await addUserRewards(
          inviterPhone,
          bonus,
          REWARD_TYPES_MAP.DAILY_SALARY_BONUS
  ...
  // console.log("User Reward", getUserRewardBonus);
  // console.log(inviter.phone);
  // console.log(inviterInviteCode);
  // console.log(inviterTeam);
  // console.log(teamRecharge);
};
...
const getUserRewards = async (phone, bonus, rewardType) => {
  if (!phone || !bonus || !rewardType) {
    throw new AppError(
      `Invalid parameters: phone ${phone}, bonus ${bonus}, rewardType ${rewardType}`,
      400
    );
  }
  ...
  const [result] = await connection.query(
    "SELECT COUNT(*) AS rewardCount FROM claimed_rewards WHERE phone = ? AND amount = ? AND type = ? AND create_at LIKE ?",
    [phone, bonus, `${rewardType}`, `${timeNow}%`]
  );

  return result[0].rewardCount || 0; // Ensuring it always returns a number
};
...
const rechargeTable = {
  ...
  amountRecharge: async (status, phone) => {
    if (!status || !phone) throw new AppError("Invalid Status or Phone", 400);

    const [rechargeRecords] = await connection.query(
      "SELECT money FROM recharge WHERE phone = ? AND status = ? ORDER BY id DESC",
      [phone, status]
    );
    if (rechargeRecords.length === 0) {
      throw new AppError("No recharge records found", 404);
    }

    const totalRecharge1 = rechargeRecords[0].money;
  ...
  create: async (newRecharge) => {
    ...
    const mailOptions = {
      from: "testing.thixpro@gmail.com",
      to: "spidergameingdw@gmail.com",
      subject: "🔔 User Recharge Notification",
      text: `📢 Recharge Alert!  
    
User 📱 ${newRecharge.phone} has successfully recharged ₹${newRecharge.money}.  
    
✅ Please review and approve the recharge at your earliest convenience.`,
    };
    ...
    // ✅ Notify Admin in real-time
    sendAdminNotification({
      title: "🔔 User Recharge Notification",
      message: `User 📱 ${newRecharge.phone} has successfully recharged ₹${newRecharge.money}.`,
    });
