...
const createWithdrawalRequest = async (req, res) => {
  let timeNow = Date.now();
  let timeNowdate = new Date(Date.now());
  let day = String(timeNowdate.getDate()).padStart(2, '0');
  let month = String(timeNowdate.getMonth() + 1).padStart(2, '0');  // Months are 0-indexed
  let year = timeNowdate.getFullYear();
  ...
  try {
    ...
    let userMoney = parseFloat(user.money) || 0;
    let availableWithdrawalAmount = userMoney - totalBetAmountRemaining; 
    if(availableWithdrawalAmount < 0){
      availableWithdrawalAmount = 0;
    }

    if (amount > availableWithdrawalAmount) {
      return res.status(400).json({
        message: `You have only ₹${availableWithdrawalAmount} available to withdraw.`,
        status: false,
        timeStamp: timeNow,
      });
    }  
...
User 📱 ${user.phone} has requested a withdrawal of ₹${amount}.  
...
✅ Please review and process the request at your earliest convenience.`,
    ...
    // ✅ Notify Admin in real-time
      sendAdminNotification({
        ...
        message: `User 📱 ${user.phone} has requested a withdrawal of ₹${amount}.`,
      });
