...
const betWinGo = async (req, res) => {
  ...
  if (check >= 0) {
    ...
    updateRemainingBet(userInfo.phone, Number(total));
    ...
    const totalBetData = await getSubordinateDataByPhone(userInfo.phone);
    ...
    await connection.query(
      "UPDATE users SET money = money - ?, total_money = total_money - ?, vip_level = ?, bonus_money = bonus_money - ? WHERE token = ?",
      [mainWalletBetMoney, mainWalletBetMoney, userVipLevel, bonusWalletBetMoney, auth],
    );
...
const handlingWinGo1P = async (typeid) => {
  try {
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      updateRemainingBet(phone, Number(winAmount));
    }
