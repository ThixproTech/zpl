...
const getCommissionStatsByTime = async (time, phone) => {
  ...
  const [commissionRow] = await connection.execute(
    `
      SELECT
          time,
          SUM(COALESCE(c.amount, 0)) AS total_commission,
          SUM(CASE 
              WHEN c.time >= ? 
...
const subordinatesDataAPI = async (req, res) => {
  try {
    ...
    const yyyy = Dyesterday.getFullYear();
    const mm = String(Dyesterday.getMonth() + 1).padStart(2, '0'); // Months are 0-based
    const dd = String(Dyesterday.getDate()).padStart(2, '0');
    
    const DformattedDate = `${yyyy}-${mm}-${dd}`;
    console.log(DformattedDate);

    const [commissionYesturdayRow] = await connection.execute(
      "SELECT SUM(amount) AS total FROM `claimed_rewards` WHERE `phone` = ? AND DATE(`create_at`) = ?",
      [user.phone, DformattedDate],
    );


    const totalCommissionsYesterday = commissionYesturdayRow[0]?.total || 0;
    console.log(totalCommissionsYesterday);
...
const subordinatesAPI = async (req, res) => {
  try {
    const authToken = req.cookies.auth;
    const [userRow] = await connection.execute("SELECT code, phone, invite FROM users WHERE token = ? AND veri = 1", [authToken]);
    ...
    const startDate = type === "today" ? getTodayStartTime() : type === "yesterday" ? startOfYesterdayTimestamp : type === "this month" ?                     startOfMonthTimestamp : "";
    //console.log(startDate,"startDate");
    const endDate = type === "today" ? new Date().getTime() : type === "yesterday" ? endOfYesterdayTimestamp : type === "this month" ?                    endOfMonthTimestamp : "";
    ...
    console.log(level1Referrals,"level1Referrals")
    
    const users = level1Referrals && level1Referrals
      .map((user) => {
...
const InvitationBonusList = [
  ...
  {
    id: 13,
    numberOfInvitedMembers: 20000,
    numberOfDeposits: 20000,
    amountOfRechargePerPerson: 500,
    bonusAmount: 1555555,
  },
  {
    id: 14,
    numberOfInvitedMembers: 50000,
    numberOfDeposits: 50000,
    amountOfRechargePerPerson: 500,
    bonusAmount: 3555555,
  },
  {
    id: 15,
    numberOfInvitedMembers: 100000,
    numberOfDeposits: 100000,
    amountOfRechargePerPerson: 500,
    bonusAmount: 7555555,
  },
];
    ...
    const invitationBonusData = InvitationBonusList.map((item) => {
      ...
      return {
        ...
        isClaimed: claimedRewardsRow.some(
          (claimedReward) => parseFloat(claimedReward.reward_id) === item.id,
        ),
        required: {
...
const getFirstRechargeRewords = async (req, res) => {
  try {
    ...
    const [rechargeRow] = await connection.execute(
      "SELECT * FROM `recharge` WHERE `phone` = ? AND `status` = ? AND `first_deposite_bonus_status` = ? ORDER BY id DESC LIMIT 1 ",
      [user.phone, PaymentStatusMap.SUCCESS, 0],
    );
...
const claimFirstRechargeReword = async (req, res) => {
  try {
    ...
    await connection.execute(
      "UPDATE `users` SET `money` = `money` + ?, `total_money` = `total_money` + ?, `first_deposite_bonus_level_complited` = IFNULL(CONCAT(IFNULL(`first_deposite_bonus_level_complited`, ''), IF(`first_deposite_bonus_level_complited` IS NULL OR `first_deposite_bonus_level_complited` = '', ?, CONCAT(',', ?))), ?) WHERE `phone` = ?",
      [bonusAmount, bonusAmount, userLevel, userLevel, userLevel, user.phone]
    ); 
    
    await connection.execute(
      "UPDATE `recharge` SET `first_deposite_bonus_status` = ?, `first_deposite_bonus_level` = ?, remaining_bet = remaining_bet + ? WHERE `phone` = ? ORDER BY `id` DESC LIMIT 1",
      [1, userLevel + 1, bonusAmount, user.phone],
    );
...
const claimAttendanceBonus = async (req, res) => {
  try {
    ...
    const claimedBonusData = AttendanceBonusList.find((item) => item.id === attendanceBonusId);
  ...
  try {
    ...
    if (startDate && endDate) {
      const [claimedRewardsHistory] = await connection.execute(
        "SELECT * FROM `claimed_rewards` WHERE `phone` = ? AND type = ? AND time BETWEEN ? AND ?",
        [user.phone, "MISSION BONUS", `${startDate}`, `${endDate}`],
      );
      claimedRewardsRow = claimedRewardsHistory;

    } else {
      const [claimedRewardsHistory] = await connection.execute(
        "SELECT * FROM `claimed_rewards` WHERE `phone` = ? AND type = ? ",
        [user.phone, "MISSION BONUS",],
      );
      claimedRewardsRow = claimedRewardsHistory;
